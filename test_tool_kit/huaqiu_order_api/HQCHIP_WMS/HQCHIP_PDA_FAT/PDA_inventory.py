...
class PdaInventory:
    def __init__(self, pda_rss):
        ...
        # self.InventoryNo = getattr(Data, 'InventoryNo')
    ...
    def pda_inventory(self):
        ...
        if search_res["result"] != []:
            ...
            for i in range(len(self.labelNumber)):
                # 根据库位号，标签号查询盘点信息
                label_location_search_url = ("{}/wms/warehouse/pda/inventoryCheck/getSingleInvCheckRecord?inventoryNo={}&locationCode={}&labelNumber={}".
                                             format(self.WMS_URL, self.InventoryNo, self.locationCode[i], self.labelNumber[i]))
