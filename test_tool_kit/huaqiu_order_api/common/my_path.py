...
dos_stock_txt_sql = os.path.join(hc2018Admin_dir, r"stock_sql_text/text_sql.sql")
...
logo_files = []
logo_txts = []
if os.path.exists(subject_logo_dir):
    logo_files = os.listdir(subject_logo_dir)
    for file in logo_files:
        position = os.path.join(subject_logo_dir, file)
        logo_txts.append(position)
img_dir = os.path.join(subject_logo_dir, logo_files[0]) if logo_files else ""
# print(logo_txts)
# 专题图片信息
